...
外部观点若以同花顺板块代码（`.TI`）提出技术查证，程序不会把该代码直接传给行情接口。它先读取
`index_classify(src=SW2021)`，按完整板块名称选择唯一的申万 `.SI` 行业指数作为执行代理（同名时优先
一级；没有同名一级时允许唯一的下级行业）；无法唯一匹配时失败关闭。生成的证据最终仍归属原始
`.TI` 板块，并在限制中明确记录实际使用的 `.SI`
代理代码，避免把两个分类体系悄悄视为同一个标的。
...
`calculate_relative_strength` 是双主体结果：目标与显式基准都会出现在 `source_subjects`。因此它既
可以支持“目标相对基准的超额表现”，也可以支持同一调用中基准自身的区间收益事实；程序仍会逐个
核对主体代码和类型，不会授权第三个未参与计算的标的。
