...
模型调用前，程序会按本轮真实结果生成动态 JSON Schema：每日模式只允许快照内授权的股票代码、
快照调用编号和实际 `record_key`；定向审阅只允许状态为 `ok/partial` 且不是身份查询的调用编号，
以及这些结果中真实出现的行键。即使模型仍输出越权引用，本地 post-validation 也会触发一次带字段
错误提示的结构化修复；修复后仍不满足逐行“记录—标的”关系的草稿继续失败关闭。
