...
- 建议输出模式，以及激进、保守、共识建议（缺失时为 `null`）；
- 辩论图启用时的共识门、共识组装摘要和被排除意见；
...
- `DUAL_RECOMMENDATIONS_READY`：无辩论第一版已确认两位经理的独立建议，并把二者共同作为正式输出；
- `NO_ACTIONABLE_CONSENSUS`：保留两位经理原始建议，但共识组装明确判断缺少可执行共识；
- `INCOMPLETE`：上游提前结束或关键建议尚未生成。

`recommendations.output_mode` 进一步区分建议区的解释方式：

- `DUAL_INDEPENDENT`：激进和保守建议都是正式输出，不声称二者已达成共识；
- `CONSENSUS`：执行过辩论链，另有共识建议或明确的无可执行共识结果；
- `INCOMPLETE`：尚未满足上述任一完整输出契约。
...
- 双独立模式不会渲染“没有未决分歧”，因为该模式根本没有执行分歧判断；
